...
prism-adapters-in   — REST, MCP server, alert webhook; depends on domain
...
- **Schema discovery & seeding** — to stop the model guessing label/metric/tag names (e.g. `service` vs OTel-native `service_name`), `InvestigationLoop` **seeds** the schema names (log labels, metric names, trace tags) as the first signals of every investigation, best-effort, so the model sees the real names before its first query. The model never *re-discovers* names on demand — that's seed-only — but it can discover a chosen label's or tag's **values** (which can't be seeded — too combinatorial) via the on-demand value steps (`ListLogLabelValues`, `ListTraceTagValues`), exposed as the `list_log_label_values` / `list_trace_tag_values` tools and backed by the datasource endpoints (Loki `/labels` & `/label/{n}/values`, Prometheus `/label/__name__/values`, Tempo v2 `/api/v2/search/tags` & `/tag/{t}/values`). All of these results are `SignalType.SCHEMA` signals (no dashboard link), kept fully visible in the reasoning prompt (never truncated or omitted) so the names aren't wastefully re-queried.
...
- **Alert webhook (inbound)** — `AlertWebhookController` exposes `POST /alerts` accepting the **Alertmanager-format** webhook (Prometheus Alertmanager, or Grafana's built-in alerting via a webhook contact point — both POST the same shape). `AlertMapper` turns each *firing* alert into an `InvestigationRequest` (`RequestSource.ALERT`) and submits it through the **same** async `InvestigationCommandsUseCase.submit` REST and MCP drive, so the loop, tracing (the investigation trace links back to the `POST /alerts` request span) and memory apply unchanged; resolved alerts are skipped. Translation only — no business logic. Guarded by an optional shared-secret `X-API-Key` filter (`prism.alerts.api-key`, blank = open for dev), mirroring the MCP key. Deliberately **webhook-direct, not Kafka** — no broker/Schema Registry/Avro; the trade-off is no durability (an alert firing while prism is down is lost), re-introducible behind the same `submit` spine later if buffering is needed.
...
- **Self-observability** — the app exports its own traces, logs and metrics via OTLP to the collector (→ Tempo / Prometheus / Loki). **OpenTelemetry is the single tracing façade**: the `opentelemetry-spring-boot-starter` provides the SDK, auto-instrumentation (HTTP-server, JDBC) and OTLP export with log↔trace correlation (logs carry the active span's `trace_id`). Domain-specific spans are created via the OpenTelemetry `Tracer` in `Observed*` decorators colocated with the port family each one wraps: `ObservedInvestigationRunner` → `prism.investigation` (the root), `ObservedReasoningPort` → `prism.reasoning.step` (per model decision, tagged with the `model.id` that answered), `ObservedHttpExecutor` → `prism.telemetry.query`, `ObservedEmbeddingModel` → `prism.embedding`; each makes its span current so child spans and logs nest correctly. The investigation runs async, so `prism.investigation` is its **own** root trace — `ObservedInvestigationRunner` links it back to the originating request span (a span link + `request.trace_id`), and the inbound REST/MCP adapters stamp `investigation.id` on the request span, so request and investigation traces cross-reference by `investigation.id`. The same decorators also emit `prism.*.duration` histograms via the OpenTelemetry `Meter` (tagged by outcome / step kind / backend); `micrometer-registry-otlp` contributes only the JVM/HTTP-server baseline metrics. Wired at the composition root by injecting `OpenTelemetry`. **Do NOT add `micrometer-tracing-bridge-otel`** — it is a competing tracer that silently suppresses these spans. Never put telemetry code in the domain or application layers — instrument with decorators that wrap the ports. The collector (`scripts/otel-collector-config.yaml`) also fans the traces out to **Langfuse** for agent evaluation, on a dedicated pipeline that keeps only `ai.*`-scope spans (name custom tracers `ai.prism.*` so they qualify) and forwards each service's own `langfuse-authorization` header (per-service Langfuse project) with per-credential batching — the app stays unaware, exporting OTLP once.
...
ReasoningStep (sealed):
  ...
  ListLogLabelValues | ListTraceTagValues                        → schema value discovery (SCHEMA signal)
